...
app.controller('cartsListController', function ($scope, $rootScope, cartService) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            { field: 'productRow', displayName: 'Product' },
            { field: 'count', width: 80, displayName: 'Count' },
            { field: 'worthPerItem', displayName: 'Worth/Item' },
...
app.controller('alertMessagesController', function ($scope) {
        ...
        $scope.alerts = [
            { type: 'success', msg: 'Cart row saved successfully!' }
        ];
    });

        $scope.$on('cartDeleted', function () {
        $scope.alerts = [
            { type: 'success', msg: 'Cart row deleted successfully!' }
        ];
    });
