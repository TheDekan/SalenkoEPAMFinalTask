...
app.controller('usersListController', function ($scope, $rootScope, userService) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            { field: 'name', displayName: 'Емейл' },
            { field: 'role', displayName: 'Роль' },
            { field: 'joinDate', displayName: 'Дата регистрации' },
            { field: 'blocked', displayName: 'Блокирован' }          
        ],
...
app.controller('alertMessagesController', function ($scope) {
        ...
        $scope.alerts = [
            { type: 'success', msg: 'Статус пользователя успешно изменен!' }
        ];
    });

        $scope.$on('error', function () {
        $scope.alerts = [
            { type: 'danger', msg: 'Проблема на сервере!' }
        ];
    });
