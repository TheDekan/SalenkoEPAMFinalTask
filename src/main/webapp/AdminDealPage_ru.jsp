...
<script src="js/adminDeal_ru.js"></script>
</head>
...
<body>
	...
	<div style="width: 100px; display:inline-block; float:left">
		<div id="wrapper">
			<ul class="menu2">
				<li><a href="Deal?action=deals&locale=eng"><img src="https://purepetfood.co.uk/catalog/view/theme/pure/image/uk-small.png" style="height: 20px; width: 40px"/></a></li>
				<li><a href="Deal?action=deals&locale=ru"><img src="https://upload.wikimedia.org/wikipedia/en/thumb/f/f3/Flag_of_Russia.svg/1200px-Flag_of_Russia.svg.png" style="height: 20px; width: 40px"/></a></li>
			</ul>
		</div>
	</div>
	...
	<h1>Страница контроля сделок</h1>
	...
	<div class="form2" ng-controller="dealFormController">
		...
		<div>
			<form ng-if="deal.id != null" name="dealForm"
				...
				<div class="form-group"
					ng-class="{'has-error' : dealForm.status.$invalid && dealForm.status.$dirty}">
					<label for="status">Статус:</label> <span
						ng-class="{'glyphicon glyphicon-ok' : dealForm.status.$valid && dealForm.status.$dirty}"></span>

					<select id="status" name="status" class="form-control"
						ng-model="deal.status">
				...
				<div class="buttons">
					...
					<button type="submit" class="btn btn-primary"
						ng-disabled="dealForm.$invalid">Сохранить</button>
				</div>
